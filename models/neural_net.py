...
class NeuralNet(BaseEstimator, ClassifierMixin):
    def __init__(self, net, n_classes=2, learning_rate=1e-3, batch_size=128, preprocessing=None, width=None, verbose=0, n_steps=1000, save_step=100, cuda=True):
        super().__init__()
        self.name = 'NeuralNet'
    ...
    def get_name(self):
        name = "{}-{}-{}-{}".format(self.name, self.learning_rate, self.width, self.n_steps)
        return name
    ...
    def fit(self, X, y, sample_weight=None, n_steps=None, n_epochs=None):
        ...
        if n_steps is not None:
            self._fit_steps(X, y, sample_weight, batch_size, n_steps)
        if n_epochs is not None:
            self._fit_epochs(X, y, sample_weight, batch_size, n_epochs)
        
        return self
        
    def _fit_epochs(self, X, y, sample_weight, batch_size, n_epochs):
        for epoch in range(n_epochs):
            ...
            for i, (X_batch, y_batch, w_batch) in enumerate(batch_gen):
                self._fit_batch(X_batch, y_batch, w_batch, batch_gen)
                
    def _fit_steps(self, X, y, sample_weight, batch_size, n_steps):
    ...
    def _fit_batch(self, X_batch, y_batch, w_batch, batch_gen):
    ...
    def predict_proba(self, X, batch_size=1024):
        if self.preprocessing is not None:
            X, y, sample_weight = self.preprocessing(X, None, None, training=False)
